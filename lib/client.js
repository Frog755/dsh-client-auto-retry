// dsh-client-auto-retry client half — DSH ModuleLoader 格式
// 监听会话事件流, 检测回合被中断/出错/超长时自动发送「继续」。
// 设置卡片通过 settings.general.item 注册（参照 dsh-wallpaper 模式）。
//
// 事件协议:
//   api.events.mux({}, signal) -> 帧: { type: "session/event", sessionId, event: { type, data } }
//   turn/end 事件的 data.reason.kind ∈ { completed, aborted, blocked, interrupted, error, max-tokens }

window.__ModuleLoader__.load({
  id: "@frog755/dsh-client-auto-retry",
  factory: (require) => {
    var module = { exports: {} };
    var exports = module.exports;
    Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });
    const React = require("react");
    const runtime = require("@deepseek-ai/dsh-client-runtime/client");

    const NS = "auto-retry";

    const DEFAULTS = {
      graceMs: 5000,
      cooldownMs: 20000,
      maxConsecutive: 4,
      maxBackoffMs: 300000,
      continueText: "继续",
      // 自身回显窗口: 自动发送「继续」后, 在此窗口内收到内容相同的 user/message
      // 视为插件自己的回显 (sessions.prompt 的 source.kind 也是 "user"), 不计为人工输入。
      echoWindowMs: 30000,
      scanOnBoot: true,
      freshMs: 15 * 60 * 1000,
      verbose: true
    };

    const FIELDS = [
      { field: "graceMs", label: "宽限期 (ms)", hint: "中断后等多少毫秒再自动发送「继续」", type: "number" },
      { field: "cooldownMs", label: "冷却期 (ms)", hint: "同一会话两次自动继续的最小间隔", type: "number" },
      { field: "maxConsecutive", label: "最多连续次数", hint: "连续自动继续多少次后停下等人工介入", type: "number" },
      { field: "maxBackoffMs", label: "退避上限 (ms)", hint: "第 n 次重试等 graceMs*2^(n-1)，封顶此值，防连续失败立刻续烧", type: "number" },
      { field: "continueText", label: "继续文本", hint: "自动发送的文本内容", type: "text" },
      { field: "echoWindowMs", label: "回显窗口 (ms)", hint: "自动发送后多少毫秒内收到同名文本视为自身回显, 不计为人工输入", type: "number" },
      { field: "scanOnBoot", label: "启动时扫描", hint: "页面加载时扫描最近被中断的会话", type: "bool" },
      { field: "freshMs", label: "扫描窗口 (ms)", hint: "只恢复此时间段内被中断的会话", type: "number" },
      { field: "verbose", label: "调试日志", hint: "在控制台输出 [auto-retry] 日志", type: "bool" }
    ];

    function resolveConfig(value) {
      const v = value && typeof value === "object" ? value : {};
      const out = {};
      for (const key of Object.keys(DEFAULTS)) {
        const val = v[key];
        out[key] = val === undefined || val === null ? DEFAULTS[key] : val;
      }
      return out;
    }

    function log(verbose, msg) {
      if (verbose) console.log("[auto-retry] " + msg);
    }

    // 把 user/message 的 content (数组 parts 或字符串) 展平成纯文本, 用于识别自身回显。
    function contentToText(content) {
      if (content == null) return "";
      if (typeof content === "string") return content;
      if (Array.isArray(content)) {
        return content.map(function (block) {
          if (!block || typeof block !== "object") return "";
          if (block.type === "text" && typeof block.text === "string") return block.text;
          return "";
        }).join("").trim();
      }
      return "";
    }

    // 状态变更通知: 输入区停止/恢复按钮用 (模块级, 与 runner 生命周期同进退)。
    const changeListeners = new Set();
    function emitChange() {
      for (const fn of changeListeners) {
        try { fn(); } catch (err) {}
      }
    }

    async function pumpStream(open, onFrame, onReconnect, backoffMs, verbose, signal) {
      let attempts = 0;
      while (!signal.aborted) {
        attempts++;
        try {
          console.log("[auto-retry] 尝试连接 mux, 第 " + attempts + " 次");
          const stream = open(signal);
          const isAsyncIter = stream && typeof stream[Symbol.asyncIterator] === "function";
          console.log("[auto-retry] mux 返回类型: " + (typeof stream) + (isAsyncIter ? " (async iterable)" : " (NOT iterable!)"));
          let frameCount = 0;
          if (!isAsyncIter) {
            console.log("[auto-retry] mux 不是 async iterable, 无法迭代");
            return;
          }
          for await (const frame of stream) {
            frameCount++;
            if (signal.aborted) return;
            try { onFrame(frame); } catch (err) { log(verbose, "frame error: " + err.message); }
          }
          console.log("[auto-retry] mux 流自然结束, 共收到 " + frameCount + " 帧");
        } catch (err) {
          console.log("[auto-retry] mux 流错误: " + (err && err.message ? err.message : String(err)));
        }
        if (signal.aborted) return;
        log(verbose, "stream closed, reconnecting in " + backoffMs + "ms");
        await new Promise((resolve) => {
          const timer = setTimeout(resolve, backoffMs);
          signal.addEventListener("abort", () => { clearTimeout(timer); resolve(); }, { once: true });
        });
      }
    }

    class AutoRetryRunner {
      constructor(api, getConfig) {
        this.api = api;
        this.getConfig = getConfig;
        this.states = new Map();
        this.abort = new AbortController();
        this._scanTimer = null;
        this._started = false;
      }

      state(sessionId) {
        let s = this.states.get(sessionId);
        if (!s) {
          s = {
            consecutive: 0,
            lastAttemptAt: 0,
            pendingTimer: undefined,
            queued: 0,
            running: false,
            subagent: false,
            stopped: false,
            lastAutoAt: 0,
            lastEndKind: null
          };
          this.states.set(sessionId, s);
        }
        return s;
      }

      start() {
        if (this._started) return;
        this._started = true;
        const config = this.getConfig();
        void pumpStream(
          (signal) => this.api.events.mux({}, signal),
          (frame) => this.onMuxFrame(frame),
          () => this.scanInterrupted(),
          Math.min(config.cooldownMs || 20000, 5000),
          config.verbose,
          this.abort.signal
        );
        if (config.scanOnBoot) {
          this._scanTimer = setTimeout(() => void this.scanInterrupted(), 5000);
        }
      }

      dispose() {
        this.abort.abort();
        if (this._scanTimer) clearTimeout(this._scanTimer);
        for (const s of this.states.values()) {
          if (s.pendingTimer) clearTimeout(s.pendingTimer);
        }
        this.states.clear();
      }

      onMuxFrame(frame) {
        const config = this.getConfig();
        // mux 帧是 RPC envelope: { rpcId, payload }, 实际内容在 payload 里
        const payload = (frame && frame.payload) || frame;
        if (!payload || payload.type !== "session/event") return;
        const sessionId = payload.sessionId;
        if (!sessionId) return;
        const state = this.state(sessionId);
        const event = payload.event;
        if (!event || !event.type) return;

        if (event.type === "turn/start") {
          state.running = true;
          this.cancelPending(sessionId);
        } else if (event.type === "turn/end") {
          state.running = false;
          this.cancelPending(sessionId);
          const reason = event.data && event.data.reason;
          const kind = reason && reason.kind;
          const errCode = reason && reason.error ? (reason.error.code || "") : "";
          const errMsg = reason && reason.error ? (reason.error.message || "") : "";
          state.lastEndKind = kind || null;
          log(config.verbose, "turn/end kind=" + String(kind) + " err=" + String(errCode) + " " + String(errMsg).slice(0, 80));
          if (kind === "completed" || kind === "aborted" || kind === "blocked") {
            state.consecutive = 0;
            if (kind === "completed") {
              // 一轮正常完成: 退出「已停止」状态, 自动重试重新武装。
              if (state.stopped) log(config.verbose, "turn 正常完成, 恢复自动重试 " + sessionId);
              state.stopped = false;
            }
            emitChange();
            return;
          }
          if (kind === "error" || kind === "interrupted" || kind === "max-tokens") {
            this.schedule(sessionId, "turn/end:" + kind);
          } else {
            log(config.verbose, "turn/end kind=" + String(kind) + " 未处理");
          }
        } else if (event.type === "user/message") {
          if (event.data && event.data.source && event.data.source.kind === "user") {
            const text = contentToText(event.data.content);
            // 自身回显判定: 自动发送的「继续」其 source.kind 同样是 "user"
            // (sessions.prompt 的 MessageSource kind 保持 'user'), 只能靠
            // 「文本相同 + 发送后窗口内」识别, 否则它会把自己的计数清零导致上限失效。
            const isSelfEcho = text === config.continueText &&
              (Date.now() - state.lastAutoAt) < config.echoWindowMs;
            if (isSelfEcho) {
              log(config.verbose, "忽略自身「" + config.continueText + "」回显 " + sessionId);
              return;
            }
            // 真正的人工输入 (或来自其他插件的用户角色消息):
            // 若正处于自动重试循环中 (已发过继续或有待发计时器) 则整条退出, 等人工接管。
            const inRetryLoop = state.consecutive > 0 || state.pendingTimer !== undefined;
            state.consecutive = 0;
            this.cancelPending(sessionId);
            if (inRetryLoop) {
              state.stopped = true;
              log(config.verbose, "检测到人工输入, 停止自动重试 " + sessionId);
            }
            emitChange();
          }
        }
      }

      schedule(sessionId, reason) {
        const config = this.getConfig();
        const state = this.state(sessionId);
        if (state.subagent) return;
        if (state.stopped) {
          log(config.verbose, "跳过 " + sessionId + "(" + reason + "): 已手动停止自动重试");
          return;
        }
        if (state.pendingTimer !== undefined) return;
        if (Date.now() - state.lastAttemptAt < config.cooldownMs) return;
        if (state.consecutive >= config.maxConsecutive) {
          log(config.verbose, "跳过 " + sessionId + "(" + reason + "): 已连续自动继续 " + state.consecutive + " 次, 等待人工");
          return;
        }
        if (state.queued > 0) return;

        // 指数退避: 第 n 次重试 (consecutive=n) 等待 graceMs * 2^n, 封顶 maxBackoffMs。
        // 首次 (n=0) 保持原 graceMs 行为; 连续失败时等待时间翻倍, 避免立刻续烧 token。
        const delay = Math.min(config.graceMs * Math.pow(2, state.consecutive), config.maxBackoffMs);
        const timer = setTimeout(() => {
          if (state.pendingTimer !== timer) return;
          state.pendingTimer = undefined;
          void this.fire(sessionId, reason);
        }, delay);
        state.pendingTimer = timer;
        log(config.verbose, "检测到中断 " + sessionId + "(" + reason + "), " + delay + "ms 后自动发送「" + config.continueText + "」(连续第 " + (state.consecutive + 1) + " 次, 退避中)");
        emitChange();
      }

      cancelPending(sessionId) {
        const state = this.state(sessionId);
        if (state.pendingTimer === undefined) return;
        clearTimeout(state.pendingTimer);
        state.pendingTimer = undefined;
      }

      // ---- 手动退出/恢复 (输入区小按钮) ----

      stopRetry(sessionId) {
        const state = this.state(sessionId);
        state.stopped = true;
        this.cancelPending(sessionId);
        log(true, "手动停止自动重试 " + sessionId + " (已连续 " + state.consecutive + " 次)");
        emitChange();
      }

      resumeRetry(sessionId) {
        const state = this.state(sessionId);
        state.stopped = false;
        state.consecutive = 0;
        log(true, "恢复自动重试 " + sessionId);
        // 上一轮若以中断结尾, 恢复后立即重新排期 (重新武装)。
        if (state.lastEndKind === "error" || state.lastEndKind === "interrupted" || state.lastEndKind === "max-tokens") {
          this.schedule(sessionId, "resume:" + state.lastEndKind);
        }
        emitChange();
      }

      async fire(sessionId, reason) {
        const config = this.getConfig();
        const state = this.state(sessionId);
        if (state.running) return;
        if (state.queued > 0) return;
        if (state.subagent) return;
        if (state.stopped) return;

        state.lastAttemptAt = Date.now();
        state.lastAutoAt = Date.now();
        try {
          const response = await this.api.sessions.prompt({
            sessionId: sessionId,
            mode: "queue",
            content: [{ type: "text", text: config.continueText }]
          });
          // 无论发送成败都计入连续次数, 保证 maxConsecutive 在失败路径同样生效。
          state.consecutive += 1;
          if (response && response.result && response.result.ok) {
            log(config.verbose, "已自动发送「" + config.continueText + "」到 " + sessionId + "(" + reason + "), 第 " + state.consecutive + " 次");
          } else {
            const err = response && response.result && response.result.error;
            log(config.verbose, "发送失败 " + sessionId + ": " + (err ? err.code + " " + err.message : "unknown") + " (第 " + state.consecutive + " 次)");
          }
        } catch (err) {
          state.consecutive += 1;
          log(config.verbose, "发送异常 " + sessionId + ": " + err.message + " (第 " + state.consecutive + " 次)");
        }
        emitChange();
      }

      async scanInterrupted() {
        const config = this.getConfig();
        if (!config.scanOnBoot) return;
        try {
          const resp = await this.api.sessions.list({});
          if (!resp || !resp.result || !resp.result.ok) return;
          const sessions = resp.result.value || resp.result.data || [];
          if (!Array.isArray(sessions)) return;
          const now = Date.now();
          let scanned = 0;
          for (const s of sessions) {
            if (scanned >= 8) break;
            const sessionId = s.id || s.sessionId;
            if (!sessionId) continue;
            const state = this.state(sessionId);
            if (state.subagent) continue;
            const updated = s.updatedAt || s.lastActivityAt;
            if (!updated) continue;
            const updatedMs = typeof updated === "number" ? updated : Date.parse(updated);
            if (isNaN(updatedMs)) continue;
            if (now - updatedMs > config.freshMs) continue;
            scanned += 1;
            this.schedule(sessionId, "scan:interrupted");
          }
          log(config.verbose, "扫描完成, 检查 " + scanned + " 个最近会话");
        } catch (err) {
          log(config.verbose, "扫描失败: " + err.message);
        }
      }
    }

    // ---------- 设置卡片 (参照 dsh-wallpaper 模式) ----------
    const zh = {
      "autoRetry.title": "断联自动重试",
      "autoRetry.hint": "检测到回合中断/出错/超长时自动发送「继续」",
      "autoRetry.save": "保存",
      "autoRetry.saved": "已保存",
      "autoRetry.saving": "保存中…",
      "autoRetry.reset": "重置为默认",
      "autoRetry.overridden": "已覆盖",
      "autoRetry.readonly": "只读部署, 无法修改"
    };
    const en = {
      "autoRetry.title": "Auto Retry",
      "autoRetry.hint": "Auto-send 「继续」 when a turn is interrupted/errored/overlong",
      "autoRetry.save": "Save",
      "autoRetry.saved": "Saved",
      "autoRetry.saving": "Saving…",
      "autoRetry.reset": "Reset to default",
      "autoRetry.overridden": "Overridden",
      "autoRetry.readonly": "Read-only deployment"
    };

    // 主题化样式：跟随 DSH 明/暗主题 token，不再硬编码黑底。
    let styleEl = null;
    function ensureStyles() {
      if (styleEl !== null && document.head.contains(styleEl)) return;
      styleEl = document.createElement("style");
      styleEl.textContent = [
        // 输入框（text/number）与下拉共用
        ".ar2-input{--tw:var(--dsw-alias-border-l2);box-sizing:border-box;width:100%;margin-top:4px;padding:6px 10px;border:1px solid var(--tw);border-radius:8px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:inherit;font-size:13px;line-height:20px;outline:none;transition:border-color .15s ease,box-shadow .15s ease}",
        ".ar2-input:hover{border-color:var(--dsw-alias-label-dimmed)}",
        ".ar2-input:focus{--tw:var(--dsw-alias-brand-primary);border-color:var(--dsw-alias-brand-primary);box-shadow:0 0 0 2px color-mix(in srgb,var(--dsw-alias-brand-primary) 20%,transparent)}",
        ".ar2-input::placeholder{color:var(--dsw-alias-label-tertiary)}",
        ".ar2-input:disabled{opacity:.55;cursor:not-allowed}",
        // checkbox 用主题强调色
        ".ar2-check{accent-color:var(--dsw-alias-brand-primary);width:16px;height:16px;margin-top:6px;cursor:pointer}",
        ".ar2-check:disabled{cursor:not-allowed;opacity:.55}",
        // 主按钮
        ".ar2-btn{display:inline-flex;align-items:center;gap:6px;padding:6px 16px;border:none;border-radius:8px;background:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-inverse,#fff);font:inherit;font-size:13px;font-weight:500;line-height:20px;cursor:pointer;transition:filter .15s ease,opacity .15s ease}",
        ".ar2-btn:hover:not(:disabled){filter:brightness(1.08)}",
        ".ar2-btn:disabled{opacity:.55;cursor:not-allowed}",
        // 输入区停止/恢复小按钮 (紧凑, 不换行)
        ".ar2-stop{display:inline-flex;align-items:center;gap:4px;height:26px;padding:0 10px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-secondary);font:inherit;font-size:12px;line-height:1;white-space:nowrap;cursor:pointer;transition:border-color .15s ease,color .15s ease,background .15s ease}",
        ".ar2-stop:hover{border-color:var(--dsw-alias-label-dimmed);color:var(--dsw-alias-label-primary)}",
        ".ar2-stop.ar2-stop-on{border-color:color-mix(in srgb,var(--dsw-alias-state-danger-primary) 55%,transparent);color:var(--dsw-alias-state-danger-primary)}",
        ".ar2-stop.ar2-stop-on:hover{background:color-mix(in srgb,var(--dsw-alias-state-danger-primary) 10%,transparent)}",
        ".ar2-stop.ar2-stop-off{border-color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 55%,transparent);color:var(--dsw-alias-state-success-primary)}",
        ".ar2-stop:disabled{opacity:.55;cursor:not-allowed}"
      ].join("");
      document.head.append(styleEl);
    }

    function AutoRetryRow(props) {
      console.log("[auto-retry] AutoRetryRow 渲染, props keys:", Object.keys(props || {}).join(","));
      const t = props.t || function (k) { return k; };
      const useStore = props.useStore || function () { return {}; };
      const setConfig = props.setConfig;
      const resetConfig = props.resetConfig;
      const config = useStore(function (state) { return state.value; }) || {};
      const value = resolveConfig(config);
      const writable = useStore(function (state) { return state.writable; }) !== false;
      const draftsState = React.useState({});
      const drafts = draftsState[0];
      const setDrafts = draftsState[1];
      const savedState = React.useState(false);
      const saved = savedState[0];
      const setSaved = savedState[1];
      const savingState = React.useState(false);
      const saving = savingState[0];
      const setSaving = savingState[1];

      function stage(field, text) {
        setDrafts(function (d) { const nd = Object.assign({}, d); nd[field] = text; return nd; });
        setSaved(false);
      }

      function display(field) {
        if (field in drafts) return drafts[field];
        const v = value[field];
        return v === undefined ? "" : String(v);
      }

      async function save() {
        setSaving(true);
        try {
          for (const f of FIELDS) {
            if (!(f.field in drafts)) continue;
            const text = String(drafts[f.field]).trim();
            let parsed;
            if (f.type === "number") {
              parsed = Number(text);
              if (!Number.isFinite(parsed)) continue;
            } else if (f.type === "bool") {
              parsed = text === "true" || text === "1";
            } else {
              parsed = text;
            }
            await setConfig(f.field, parsed);
          }
          setDrafts({});
          setSaved(true);
        } catch (err) {
          console.log("[auto-retry] 保存失败:", err);
        } finally {
          setSaving(false);
        }
      }

      const rows = FIELDS.map(function (f) {
        return React.createElement(
          "div", { key: f.field, style: { marginBottom: 8 } },
          React.createElement("label", { style: { display: "block", color: "var(--dsw-alias-label-primary)", fontWeight: 500, fontSize: 13, lineHeight: "20px" } },
            f.label
          ),
          React.createElement("input", {
            className: f.type === "bool" ? "ar2-check" : "ar2-input",
            type: f.type === "bool" ? "checkbox" : f.type === "number" ? "number" : "text",
            value: f.type === "bool" ? undefined : display(f.field),
            checked: f.type === "bool" ? display(f.field) === "true" : undefined,
            onChange: function (ev) { stage(f.field, f.type === "bool" ? String(ev.target.checked) : ev.target.value); },
            disabled: !writable
          }),
          f.hint ? React.createElement("div", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 12, lineHeight: "18px", marginTop: 3 } }, f.hint) : null
        );
      });

      return React.createElement("div", { style: { padding: "4px 2px" } },
        React.createElement("div", { style: { color: "var(--dsw-alias-label-secondary)", fontSize: 13, lineHeight: "20px", marginBottom: 4 } }, t("autoRetry.hint")),
        rows,
        React.createElement("div", { style: { marginTop: 4, display: "flex", gap: 8, alignItems: "center" } },
          React.createElement("button", {
            className: "ar2-btn",
            onClick: function () { void save(); },
            disabled: !writable || saving
          }, saving ? t("autoRetry.saving") : t("autoRetry.save")),
          saved ? React.createElement("span", { style: { color: "var(--dsw-alias-state-success-primary)", fontSize: 12 } }, t("autoRetry.saved")) : null,
          !writable ? React.createElement("span", { style: { color: "var(--dsw-alias-state-warn-primary)", fontSize: 11 } }, t("autoRetry.readonly")) : null
        )
      );
    }

    // ---------- 输入区停止/恢复小按钮 (conversation.input.left) ----------
    // 常驻在输入框工具行最左侧: 点击停止后本会话不再自动发送「继续」(排期与待发一并取消),
    // 再次点击恢复; 若上一轮以中断结尾, 恢复会立即重新排期。
    function AutoRetryStopButton(props) {
      const sessionId = props && props.sessionId;
      const tickState = React.useState(0);
      const setTick = tickState[1];
      React.useEffect(function () {
        function onChange() { setTick(function (t) { return t + 1; }); }
        changeListeners.add(onChange);
        return function () { changeListeners.delete(onChange); };
      }, []);
      const state = (sessionId && current) ? current.state(sessionId) : null;
      const stopped = !!(state && state.stopped);
      const count = state ? state.consecutive : 0;
      const active = !stopped && count > 0;
      const label = stopped ? "▶ 恢复自动重试" : (count > 0 ? "⏹ 停止重试 (" + count + ")" : "⏹ 停止自动重试");
      const title = stopped
        ? "已停止自动重试: 点击恢复 (若上一轮中断会立即重新排期)"
        : "停止自动重试: 不再自动发送「" + (resolveConfig(undefined).continueText) + "」";
      function onClick() {
        if (!current || !sessionId) return;
        if (stopped) { current.resumeRetry(sessionId); } else { current.stopRetry(sessionId); }
      }
      return React.createElement("button", {
        className: "ar2-stop" + (stopped ? " ar2-stop-off" : (active ? " ar2-stop-on" : "")),
        onClick: onClick,
        title: title,
        "data-auto-retry-stop": "1"
      }, label);
    }

    let current = null;
    let cardActions = null;
    // 渲染中 store 实例的 actions, 由 slot inject 回调在挂载时绑定。
    // 必须用它更新卡片读数; store.create() 每次都会新建无人渲染的实例。
    let liveActions = null;

    function apply(ctx) {
      const api = ctx.connection && ctx.connection.api;
      if (api === undefined) {
        console.log("[auto-retry] connection.api 不可用, 跳过");
        return;
      }

      // 注入主题化样式（跟随 DSH 明/暗主题，不硬编码颜色）
      ensureStyles();

      // 绑定 settings scope
      let scope = null;
      try {
        if (ctx.settingsScope !== undefined) {
          scope = ctx.settingsScope.bind({ namespace: NS });
        }
      } catch (err) {
        console.log("[auto-retry] settingsScope 不可用:", err.message);
      }

      function getConfig() {
        if (scope) {
          try { return resolveConfig(scope.getSnapshot().value); } catch (err) {}
        }
        return resolveConfig(undefined);
      }

      // 引擎
      ctx.effect(function () {
        if (current) current.dispose();
        current = new AutoRetryRunner(api, getConfig);
        current.start();
        return function () { if (current) { current.dispose(); current = null; } };
      });

      // locale + 设置卡片 (settings.general.item, 参照 dsh-wallpaper)
      console.log("[auto-retry] 检查服务: slots=" + (ctx.slots !== undefined) + " locale=" + (ctx.locale !== undefined) + " settingsScope=" + (ctx.settingsScope !== undefined));
      try {
        ctx.effect(() => ctx.locale.register(NS, { zh, en }), "auto-retry: locale");
        console.log("[auto-retry] locale 注册已调用");
      } catch (err) {
        console.log("[auto-retry] locale 注册失败:", err.message);
      }

      try {
        const slots = ctx.slots;
        if (slots !== undefined) {
          console.log("[auto-retry] slots 可用, 开始注册卡片");
          // store 保存配置值 + writable（参照 dsh-wallpaper: defineStore({init, actions})）
          const store = runtime.defineStore({
            init: () => {
              const snap = scope ? scope.getSnapshot() : { value: undefined, writable: false };
              return { value: snap.value, writable: snap.writable !== false };
            },
            actions: {
              setValue: (draft, value) => { draft.value = value; },
              setWritable: (draft, writable) => { draft.writable = writable; }
            }
          });
          // 订阅 scope 变化更新 store：必须更新渲染中的那个实例（liveActions），
          // 而不是 store.create() 新建的无人渲染的实例——否则卡片永远显示旧值。
          if (scope) {
            ctx.effect(function () {
              return scope.subscribe(function () {
                const snap = scope.getSnapshot();
                if (liveActions !== null) {
                  liveActions.setValue(snap.value);
                  liveActions.setWritable(snap.writable !== false);
                }
              });
            });
          }
          ctx.slots.inject("settings.general.item", function () {
            console.log("[auto-retry] settings.general.item inject 回调执行");
            return ctx.slots.register({
              name: "settings.general.item",
              id: "auto-retry",
              order: 90,
              store: store,
              locale: NS,
              inject: function (bound) {
                cardActions = bound;
                // bound 是渲染中 store 实例的 actions（setValue/setWritable），
                // 订阅回调用它更新卡片读数（参照 dsh-wallpaper 的 actions.sync 模式）。
                liveActions = bound;
                return {
                  setConfig: function (field, val) {
                    if (scope) return scope.set(field, val);
                    return Promise.resolve();
                  },
                  resetConfig: function (field) {
                    if (scope) return scope.unset(field);
                    return Promise.resolve();
                  }
                };
              }
            }, AutoRetryRow);
          });
          // 输入区停止/恢复按钮 (会话级小控件, 不占整行)
          ctx.slots.inject("conversation.input.left", function () {
            console.log("[auto-retry] conversation.input.left inject 回调执行");
            return ctx.slots.register({
              name: "conversation.input.left",
              id: "auto-retry-stop",
              order: 20,
              label: function () { return "auto-retry stop"; }
            }, AutoRetryStopButton);
          });
        }
      } catch (err) {
        console.log("[auto-retry] 设置卡片注册失败:", err.message);
      }
    }

    exports.apply = apply;
    exports.inject = ["slots", "locale", "connection", "settingsScope"];
    exports.NS = NS;
    return module.exports;
  }
});
